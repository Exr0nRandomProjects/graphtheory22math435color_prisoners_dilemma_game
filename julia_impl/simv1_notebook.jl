...
@bind N_PLAYERS_POWER html"<input type=range min=1 max=80>"
...
begin
	scoring = [[[1, 1], [5, 0]],
			   [[0, 5], [3, 3]]]
	play_match(players) = (
		@assert length(players) == 2 "expects two players!";
		moves = players .|> (x -> x.color > 0 ? x.color : x.prev);
		# println(map(p -> p.color, players), moves);
		scores = scoring[moves[1]][moves[2]];
		shifted_moves = cat(moves[end], moves[1:end-1], dims=1); # all the moves but rotated by one so that each player gets the move from the play before for the prev
		map((p, s, o) -> merge(p, (score=p.score + s, prev=o)),
			players, scores, shifted_moves)
	)
	# play_match([new_Player(), new_Player()])
end

# ╔═╡ cce178a7-aab3-47a9-9303-9482688dcee2
@bind N_ITERS html"<input type=range min=0 max=100 name=N_ITERS>"
...
begin
	...
	histogram(final_scores |> Iterators.flatten .|> (p -> p.score), xlabel="score", ylabel="number of players", label="$(N_PLAYERS) random players, $(N_ITERS) rounds, no reshuffle")
end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "f4f0088c458249f0e510169fecac03da3a9ce42e"
